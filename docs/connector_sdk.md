...
标准接入先复制最接近的脱敏示例，只改来源 ID、允许目标、鉴权引用、分页、字段白名单和策略文件；不要复制真实 Token、Cookie、抓包正文或个人数据。接入前依次通过 JSON Schema、来源登记一致性、固定夹具、失败分类、幂等重放和断点续跑测试。

连接器升级时保留旧 `connector_version` 的夹具和策略，先对同一份固定输入并行比较输出契约、检查点与质量码。兼容升级只增加可选字段；字段删除、语义改变或安全默认值变化必须升级协议主版本。发布后发现漂移时，先停用该来源并回到上一版策略/worker，已有原始归档和检查点不删除。

卸载只分三步：先通过控制面停用来源，确认没有在途运行，再移除任务/策略引用。历史记录、归档、审计和版本号默认保留，除非数据保留政策另有明确删除流程。

目前“只改 ConnectorSpec 即可接入”适用于现有通用契约和独立 CLI/worker；要把一种全新的执行器加入总控台固定任务，仍需显式注册受支持的 worker，并补构建清单和测试。项目不把尚未实现的动态插件发现包装成现成功能。
